...
class _RevenueReportScreenState extends State<RevenueReportScreen> {
  ...
  Future<void> _calculateTotalSummary() async {
    ...
    try {
      ...
      // ✅ Fetch completed subscriptions for revenue (PDF + Video)
      final subSnapshot = await FirebaseFirestore.instance
          .collection('client_subscriptions')
          .where('paymentStatus', whereIn: ['completed', 'COMPLETED'])
          .get();

      double pdfSubscriptionRevenue = 0.0;
      double videoSubscriptionRevenue = 0.0;
      ...
      for (var doc in subSnapshot.docs) {
        ...
        if (subType == 'video') {
          videoSubscriptionRevenue += price.toDouble();
        } else {
          pdfSubscriptionRevenue += price.toDouble();
        }
