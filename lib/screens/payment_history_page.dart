        ...
        builder: (context, purchasesSnap) {
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, subscriptionsSnap) {
              ...
              // Add subscriptions (both PDF and Video)
              for (final doc in subscriptionDocs) {
                final data = doc.data() as Map<String, dynamic>;
                final subType = (data['type'] as String? ?? 'pdf').toLowerCase();
                final displayName = subType == 'video' ? 'Video Workouts Subscription' : 'PDF Workouts Subscription';
                ...
                allPayments.add({
                  ...
                  'planName': data['planName'] ?? displayName,
                  'price': data['price'] ?? 0,
                  ...
                  'subscriptionType': subType, // Track whether it's 'pdf' or 'video'
                });
              }
              ...
              return ListView(
                ...
                children: [
                  ...
                  ...allPayments.map((payment) {
                    ...
                    final subType = (payment['subscriptionType'] as String? ?? 'pdf').toLowerCase();
                    final category = type == 'subscription'
                        ? (subType == 'video' ? 'Video Workouts Subscription' : 'PDF Workouts Subscription')
                              ...
                              Row(
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: type == 'subscription'
                                          ? (subType == 'video' ? const Color(0xFF1C2D5E).withValues(alpha: 0.1) : Colors.purple.withValues(alpha: 0.1))
                                          : Colors.orange.withValues(alpha: 0.1),
                                      borderRadius: BorderRadius.circular(8),
                                      border: Border.all(
                                        color: type == 'subscription'
                                            ? (subType == 'video' ? const Color(0xFF1C2D5E) : Colors.purple)
                                            : Colors.orange,
                                        width: 1,
                                      ),
                                    ),
                                    child: Text(
                                      type == 'subscription'
                                          ? (subType == 'video' ? 'VIDEO SUBSCRIPTION' : 'PDF SUBSCRIPTION')
                                          : 'PLAN',
                                      style: TextStyle(
                                        color: type == 'subscription'
                                            ? (subType == 'video' ? const Color(0xFF1C2D5E) : Colors.purple)
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        Expanded(
                                          child: Text(
                                            subType == 'video'
                                                ? 'Subscription expired - renew to access Video Workouts'
