...
class _SquarePaymentPageState extends State<SquarePaymentPage> {
  ...
  Future<String> _createCheckoutSession({
    ...
    final resp = await http.post(
      Uri.parse('$_functionsBase/checkout-session'),
      headers: {
    ...
    if (resp.statusCode != 200 || data['ok'] != true) {
      throw data['error'] ?? 'Could not start checkout.';
    }
    return data['checkoutId'] as String;
  }
  ...
  Future<void> _openWebCheckout({String paymentMethod = 'card'}) async {
    ...
    try {
      ...
      if (user == null) {
        throw 'Please sign in before purchasing.';
      }
      final uid = user.uid;
      final authToken = await user.getIdToken(true);
      if (authToken == null || authToken.isEmpty) {
        throw 'Could not authenticate payment. Please sign in again.';
      }
      ...
      final checkoutId = await _createCheckoutSession(
        planId: planId,
        referenceId: refId,
        authToken: authToken,
      );
