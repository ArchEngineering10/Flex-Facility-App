    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        ...
        builder: (context) => PopScope(
          canPop: false,
          child: VideoPlayerModal(video: video, allVideos: _videoWorkouts),
        ),
      ),
    );
  }
...
class _VideoPlayerModalState extends State<VideoPlayerModal> {
  ...
  void dispose() {
    ...
    // Only dispose if video was initialized
    if (_hasStartedPlaying) {
      try {
        // Stop video immediately
        _videoController.pause();
        _videoController.seekTo(Duration.zero);
        // Properly dispose Chewie first
        _chewieController?.dispose();
        // Then dispose video controller
        _videoController.dispose();
      } catch (_) {
        // Silently catch any errors during disposal
      }
    }
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: PopupMenuButton<String>(
          ...
          onSelected: (String value) {
            if (value == 'close') {
              // Stop video immediately before closing (only if initialized)
              if (_hasStartedPlaying) {
                try {
                  _videoController.pause();
                  _videoController.seekTo(Duration.zero);
                  ...
                  child: ClipRRect(
                    ...
                    child: Container(
                      ...
                      child: _isInitialized && _chewieController != null
                          ...
                          : GestureDetector(
                              onTap: _isLoading ? null : _initializeAndPlayVideo,
                              child: Container(
                                width: double.infinity,
