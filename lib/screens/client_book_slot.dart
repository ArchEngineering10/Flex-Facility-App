...
class _ClientBookSlotState extends State<ClientBookSlot> {
    ...
    if (isOnWaitlist) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('Remove yourself from the waitlist for $time?'),
          actions: [
      ...
      await _updateWaitlist(time, user.uid, join: false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Removed from waitlist'), behavior: SnackBarBehavior.floating),
        );
      }
    } else {
      // Join waitlist
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
            'This slot ($time) is full.\n\nJoin the waitlist and we\'ll notify you instantly if a spot opens up.',
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1C2D5E)),
      ...
      if (confirmed != true) return;
      await _updateWaitlist(time, user.uid, join: true);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text("You're on the waitlist! We'll notify you if a spot opens."),
  ...
  Future<void> _updateWaitlist(String time, String uid, {required bool join}) async {
    final selectedDate = _selectedDate ?? _focusedDate;
    final doc = await _getSlotDocForLabel(selectedDate, time);
    if (doc == null) return;

    final docRef = FirebaseFirestore.instance.collection('trainer_slots').doc(doc.id);
    ...
    // Refresh slot statuses so button updates immediately
    ...
    for (String time in slots) {
      ...
      try {
        ...
        if (doc == null || !doc.exists) {
          statuses[time] = {'isFull': false, 'isBookedByUser': false, 'isPast': false, 'isOnWaitlist': false};
        } else {
          final data = doc.data() ?? <String, dynamic>{};
          final bookedBy = List.from(data['booked_by'] ?? []);
          final waitlist = List.from(data['waitlist'] ?? []);
          statuses[time] = {
            'isFull': bookedBy.length >= slotCapacity,
            'isBookedByUser': bookedBy.contains(currentUser?.uid),
            'isPast': false,
            'isOnWaitlist': waitlist.contains(currentUser?.uid),
          };
        }
      } catch (e) {
                              ...
                              return Padding(
                                ...
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final isFull        = slotInfo['isFull'] == true;
                                    final isBookedByUser = slotInfo['isBookedByUser'] == true;
                                    final isOnWaitlist  = slotInfo['isOnWaitlist'] == true;
                                    // Full + not booked by user = show waitlist button (not disabled)
                                    ...
                                    if (isPast) {
                                      ...
                                      fgColor = const Color(0xFF1C2D5E);
                                      borderColor = const Color(0xFF1C2D5E);
                                    }
                                    ...
                                    return ElevatedButton(
                                      onPressed: isDisabled
                                          ...
                                          : isFull && !isBookedByUser
                                              ? () => _handleWaitlistTap(time, isOnWaitlist)
                                              : () => showSlotPopup(time),
                                      style: ElevatedButton.styleFrom(
